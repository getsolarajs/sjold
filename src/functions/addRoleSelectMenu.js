...
module.exports = {
    name: "$addRoleSelectMenu", description: "Adds a role select menu. Args: customID;placeholder;[minValues=1];[maxValues=1];[disabled?]", takesBrackets: true,
    execute: async (context, args) => {
        if (!args[0]) return "[Error: $addRoleSelectMenu requires a customID]";
        ...
        const selectMenu = new RoleSelectMenuBuilder().setCustomId(customId).setPlaceholder(placeholder || "Select roles...").setMinValues(minValues).setMaxValues(maxValues).setDisabled(disabled);
