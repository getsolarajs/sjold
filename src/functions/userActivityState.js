module.exports = {
    name: "$userActivityState", description: "Returns the state of the user's primary activity. Args: [userID]", takesBrackets: true,
    execute: async (context, args) => {
        ...
        try { const member = await context.guild.members.fetch(userId); return member?.presence?.activities?.[0]?.state || ""; }
